...
select region_id, count(*) as 'Countries that has comapny locations' 
	from 
		countries
	group by 
		region_id 
...
select r.region_id, r.region_name, count(c.region_id) as 'region with most locations' 
	from 
		regions r
	inner join 
...
select l.state, l.city, l.postal_code 
	from 
		locations l
	where 
...
select l.state, l.city, l.postal_code 
	from 
		locations l
	where 
		(country_id != 'US') and city like 'S%' and state is null;
...
select 
	c.country_id, c.country_name, count(l.country_id) as 'Number of locations per Country' 
    from 
		countries c
	left join 
...
select p.product_name, p.standard_cost, p.list_price 
	from 
		products p 
	where 
...
select p.product_id,p.product_name, p.list_price, pc.category_name from products p 
	inner join 
...
select p.product_name, p.list_price from products p 
    ...
    where 
		quantity > 100 and warehouse_name = 'Toronto' and pc.category_name = 'CPU';
...
select avg(list_price) as 'Average List Price' from products;
...
select pc.category_name, avg(p.category_id)
